...
**Answer** Code produces a tibble with the state, whether is voted for Trump or Clinton in 2016, percent of Trump votes, percent of businesses receiving PPE, and the dollar breakdown of PPE per employees.
```{r}
...
**Answer** There seems to be a loose correlation between states that voted for Trump and the amount of PPE loans businesses in the state get. 
```{r}
...
### Task 4: Plot data
**Task** Run the following code. Describe the plot. What do you see?
**Answer** There seems to be like there could be a very loose correlation between states that voted for Trump and the amount of PPE loans that go toward employees in those states. 
```{r}
...
**Answer** There is a tibble showing the line/dot graphs in a consolidated percentage. 
```{r}
reveal_data %>%
...
**Answer** There is a bunch of data that shows what the p-value is of the above arguments to measure the strength of the correlation. 
```{r}
...
**Answer** There is a tibble showing the mean PPP amount given per employee in states that voted for Trump and Clinton in 2016. 
```{r}
reveal_data %>%
  group_by(vote_2016) %>%
  summarise(
...
**Answer** Similar out put to p-value but measuring t-test this time.
```{r}
