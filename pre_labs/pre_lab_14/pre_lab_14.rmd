...
### Task 2: Read in data
**Task** Run the following code and describe the data frame it outputs.
**Answer** The code creates a new data frame with 12 columns and 152,584 rows. The title columns distinguishes which Chamber the person is related to and whether they are a candidate or elected official, which I think is a very helpful distinction to be aware of. 
...
**Answer** The function pulled out words that are spelled the same but have different capitalization. It reminds me of the Open Refine cleaning tool that grouped values that looked similar. I'm not totally clear where these words came from. 
...
**Answer** The code created a new data frame with one column with one-word value and 5,369,025 rows of data. 
...
**Answer** The code produces a bar graph with horizontal bars of the top 25 most popular words from the unique_words data frame. The, to & and were the top three words, which is not surprising since you need at least one of these words in every sentence to make it make sense. To and the had the highest count and passed 200,000 entries. Most of the top 25 fell in the 10,000 to 90,000 entries. 
...
**Answer** The data looks much different. Basic words are no longer at the top. The list now reflects the most popular words from the last two years at least. 
...
**Answer** Covid was obviously top of mind for most folks and this data proves that. Covid had the biggest hits in 2020 but was the top for 2020 and 2021. It seems that vaccines weren't really searched as much in 2020 as it was in 2021, which makes sense since most COVID-19 vaccines became available in 2021.  
...
**Task** Run the following code and describe the results.
**Answer** Words related to the coronavirus, the pandemic and healthcare systems have the highest percentages. Town hall is interestingg too since there were a ton of those being held on the big networks during the election. 
...
**Task** Run the following code and describe the results.
**Answer** These words reflect words related to the coronavirus, healthcare, the pandemic are in the top 10. This list, which starts after 2021 begins does not reflect news about the Big Lie, the election or Trump. This list includes one word and two word values. 
...
**Task** Run the following code and describe the results.
**Answer** This list includes one word, two word and character (i.e.ー 19 ) values. It's interesting that this list doesn't directly reflect the presidential election. 
...
**Task** Run the following code and describe the results.
**Answer** This tibble is mostly made up of two word values. These reflect politics and identity politics for conservatives and also reflects the pandemic. This tibble is much more political than the previous code block.  
...
**Answer** The code produces two data frames. Relief had the most hits in both data frames with it's attitude labeled as positive. That seems a bit counter-intuitive to me. I would personally have relief labeled as negative. You have relief when there is a tragedy or grave emergency, that's not positive to me.
