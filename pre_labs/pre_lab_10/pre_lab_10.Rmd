...
https://datawrapper.dwcdn.net/PWjJb/1/
...
We are creating a new data frame called md_counties that is pulling the number of loans per county.
...
We are creating a new data frame called md_counties that is pulling counties that received more than 6 loans.
...
The county code and state code are combined to make a fips code. 
...
Columns include county name, loan count, state and state/county/fips code. 
...
The map is now much smaller and in a weird shape to only include MD counties. 
...
The map has more detail and context so the color gradient actually makes sense. 
...
https://datawrapper.dwcdn.net/WEkGW/1/
