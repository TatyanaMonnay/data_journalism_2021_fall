...
### Task 2: Load data
**Task** Load the PPP loan dataset by running the following code. We'll use the guess_max() function as an argument to use the first 10 rows to set the data type. 

What does the first line of the red Warning message that prints out when you load the data say? Answer below.
...
**Answer** It shows that there is no loop for the break or next section and that the function is jumping to the top level
...
**Answer** Amount field and the id column are in dbl data. The name field is character data. 
```{r}
...
**Answer**: Where franchise name is NA, there are 192,959 loans. Subway has 275 loans.  
...
**Answer**: There are 135,073 loans.
...
**Answer**: There are 135,073 loans. It is the same number for the forgivenesss_amount. 
...
**Answer**: There are 135,073 loans, which is the same number from the codeblock above. I think this might mean that there could have been human error during the data entry process so there are missing/empty fields in the dataset. 
```{r}
ppp_maryland_loans %>%
...
**Answer** The three problems range from having the wrong type of data, to having gaps in the dataset and missing values.  
...
**Answer** The function added an 'x' to create "x1_id."
...
**Answer** The error says: "Error: Problem with `summarise()` column `total_amount`. ℹ `total_amount = sum(amount)`. x invalid 'type' (character) of argument Run `rlang::last_error()` to see where the error occurred." I'm not sure what it means, but I think it might be related to how the code is written or that the amount column is labeled as a character but its values are numerical. 
...
**Answer** The amount field was changed to dbl to reflect numerical values. 
...
**Answer** The total number of loans is 2,033,179.
```{r}
# cleaning function
total_arnold_md_loans <- cleaned_arnold_md_loans %>%
  summarise(total_amount = sum(amount))
...
**Answer** There are two duplicates under Stuart Jones LLC.
...
There are two misspellings remaining in the data. 
```{r}
# cleaning function
cleaned_arnold_md_loans <- arnold_md_loans %>%
  clean_names() %>%
...
**Answer** There is one misspelling left. 
